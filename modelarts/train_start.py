...
'''
11个参数：
net:网络结构 resnet50 或 resnet101 默认resnet50
dataset:数据集 cifar10 或 imagenet2012 或 rp2k 默认rp2k
run_distribute:分布式训练 默认为False
device_num:所用设备编号 默认为1 ？
# dataset_path:数据集位置 默认为空
device_target:设备类型 CPU/GPU/Ascend 默认使用Ascend
pre_trained:预训练ckpt路径 默认为空
parameter_server: 不知道是啥。。 ？
filter_weight: 
data_url: 数据集路径
train_url: 训练输出
'''
...
if __name__ == '__main__':
    ...
    mox.file.copy_parallel(args_opt.data_url, real_path, threads=0, is_processing=False)
